...
- Positions table:
  - Rental mode: Pos | Bezeichnung | Menge | Tage | EP/Tag | Gesamt
  - Simple mode: Pos | Bezeichnung | Menge | Einzelpreis | Gesamt
...
def build_angebot_pdf(
    *,
    ...
    # Simple mode: no rental/days columns, just Pos/Bezeichnung/Menge/EP/Gesamt
    ...
    if not simple_mode and start_date_str and end_date_str:
    ...
    pauschale_suffix = ""
    if not simple_mode and is_pauschale and leistungszeitraum:
        pauschale_suffix = f" (Nutzung {leistungszeitraum})"

    if simple_mode:
        # Simple mode: 5 columns – Pos | Bezeichnung | Menge | Einzelpreis | Gesamt
        col_widths = [
            22,           # Pos
            cw - 22 - 40 - 55 - 60,  # Bezeichnung (flexible)
            40,           # Menge
            55,           # Einzelpreis
            60,           # Gesamt
        ]
        ...
        for pos_idx, item in enumerate(positions):
            ...
            display_price = round(item.get("unit_price", item.get("price_per_day", 0)), 2)
            if is_regular:
                display_price = round(display_price / tax_factor, 2)
            table_data.append([
                Paragraph(str(pos_nr), styles["table_cell"]),
                Paragraph(item["name"], styles["table_cell"]),
                Paragraph(str(item["quantity"]), styles["table_cell"]),
                Paragraph(fmt_eur(display_price), styles["table_cell_right"]),
                Paragraph(fmt_eur(display_total), styles["table_cell_right"]),
            ])
            pos_nr += 1

    else:
        # Rental mode: 6 columns – Pos | Bezeichnung | Menge | Tage | EP/Tag | Gesamt
        col_widths = [
            22,           # Pos
